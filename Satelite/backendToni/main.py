...
async def analyze_all(request: URLRequest):
    ...
    try:
        ...
        fast_results = await asyncio.gather(
            ...
            _run_tool("citations", QueryCitationTracker().analyze(url)),
            _run_tool("alerts", MentionAlertAnalyzer().analyze(url)),
            return_exceptions=True,
        )
        fast_names = ["schema", "axp", "ai_presence", "citations", "alerts"]
        ...
        crawl_tools = [
            ...
            ("robots_sitemap", SiteAnalyzer(max_crawl_pages=20).analyze(url)),
            ("freshness", ContentFreshnessChecker(max_pages=15).check(url)),
