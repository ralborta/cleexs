...
# Jobs en memoria (una instancia Railway = un worker). Evita timeouts de proxy (~60s) en POST largos.
...
async def run_analyze_all_impl(url: str) -> dict:
    """Lógica compartida: análisis completo (puede tardar varios minutos)."""
    output: dict = {}
    ...
    # Crawl + robots en paralelo (ahorra ~30–40s vs en serie).
...
async def analyze_all_start(request: URLRequest):
    """Encola el análisis y devuelve al instante. Usar GET /api/analyze-all/jobs/{id} para el resultado."""
...
async def analyze_all(request: URLRequest):
    """Sincrónico (legacy). Tras varios minutos puede cortar el proxy; preferí /start + polling."""
    url = _normalize_analyze_url(request.url)
    try:
