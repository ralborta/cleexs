...
class SiteCrawler:
    def __init__(self, max_pages: int = 30, max_depth: int = 3, timeout: int = 10):
    ...
    async def crawl(self, url: str) -> CrawlResult:
        ...
        async with aiohttp.ClientSession(
            ...
            headers={"User-Agent": "CleexsCrawler/1.0 (Crawlability Check)"}
        ) as session:
            # Check robots.txt first
            await self._check_robots_txt(session)

            # Crawl pages starting from root
            await self._crawl_page(session, url, depth=0)
    ...
    async def _crawl_page(self, session: aiohttp.ClientSession, url: str, depth: int):
        ...
        try:
            ...
            async with session.get(url, allow_redirects=True) as resp:
                ...
                tasks = []
                for link in page_result.internal_links[:20]:
