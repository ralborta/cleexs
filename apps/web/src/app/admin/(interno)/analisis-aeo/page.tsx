...
type AeoDetail = AeoRow & {
  resultJson: AeoContentResult | null;
  error: string | null;
  notes: string | null;
...
function publicLink(slug: string): string {
  if (typeof window === 'undefined') return `/aeo/${slug}`;
  return `${window.location.origin}/aeo/${slug}`;
}

export default function AnalisisAeoPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await adminUiFetch('/api/admin-ui/aeo-audits');
      if (!res.ok) throw new Error((await res.json().catch(() => ({})))?.error || `Error ${res.status}`);
      const data = (await res.json()) as { items?: AeoRow[] };
      setRows(data.items ?? []);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-violet-100 bg-gradient-to-br from-violet-50/80 via-white to-fuchsia-50/60 p-6 shadow-sm md:p-8">
        <div className="absolute -right-24 -top-24 h-56 w-56 rounded-full bg-fuchsia-200/30 blur-3xl" aria-hidden />
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900 md:text-3xl">
                Análisis + Reescritura AEO
              </h1>
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">
                Analizá el contenido del sitio de un cliente y obtené un diagnóstico por dimensiones
                más una landing reescrita para que ChatGPT, Perplexity, Gemini y Claude la entiendan y
                la recomienden. Cuando esté lista, le pasás el link.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
          <div>
            <h2 className="text-sm font-semibold text-slate-900">Análisis</h2>
            <p className="text-xs text-slate-500">
              {rows.length} en total · {totalListas} listos para entregar
            </p>
          </div>
          <button
            type="button"
        ...
        {loading && rows.length === 0 ? (
          ...
          <div className="p-10 text-center">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-700">Todavía no hay análisis</p>
            <p className="mt-1 text-xs text-slate-500">Creá el primero con “Nuevo análisis”.</p>
          </div>
        ) : (
                ...
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-50 ring-1 ring-slate-200">
                  {row.scoreBefore != null ? (
                    <span className="text-xs font-bold tabular-nums text-slate-700">
                      {row.scoreBefore}
                      {row.scoreAfter != null && <span className="text-emerald-600">→{row.scoreAfter}</span>}
                    </span>
                  ) : (
...
function NewAeoForm({ onCreated, onCancel }: { onCreated: () => void; onCancel: () => void }) {
  ...
  async function submit() {
    if (!targetUrl.trim()) {
      setErr('Pegá la URL del sitio a analizar.');
      return;
    }
    setBusy(true);
    setErr(null);
    try {
      ...
      setErr(e instanceof Error ? e.message : 'Error creando el análisis');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-violet-200 bg-violet-50/30 p-6 shadow-sm">
      <h2 className="text-sm font-bold text-slate-900">Nuevo análisis AEO</h2>
      <p className="mt-0.5 text-xs text-slate-500">
        Leemos el contenido del sitio y la IA genera el diagnóstico + reescritura (~30-60s). Industria
        y región ayudan a la IA a no inventar.
      </p>
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          <label className={label}>Industria (opcional)</label>
          <input
            type="text"
            value={industry}
            onChange={(e) => setIndustry(e.target.value)}
            placeholder="Ej: software de RR.HH."
            className={field}
          />
        </div>
        <div>
          <label className={label}>Región / país (opcional)</label>
          <input
            type="text"
            value={region}
        ...
        <div className="sm:col-span-2">
          ...
          <input
            ...
            placeholder="Ej: vendido con el curso, factura #123"
            className={field}
          />
        </div>
      </div>
...
function AeoDetailView({ id, onBack }: { id: string; onBack: () => void }) {
  const [audit, setAudit] = useState<AeoDetail | null>(null);
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      const res = await adminUiFetch(`/api/admin-ui/aeo-audits/${id}`);
      if (!res.ok) throw new Error((await res.json().catch(() => ({})))?.error || `Error ${res.status}`);
      const data = (await res.json()) as { item: AeoDetail };
      setAudit(data.item);
    } catch (e) {
  ...
  async function rerun() {
    setActing(true);
    try {
      await adminUiFetch(`/api/admin-ui/aeo-audits/${id}/run`, { method: 'POST' });
  ...
  async function remove() {
    ...
    await adminUiFetch(`/api/admin-ui/aeo-audits/${id}`, { method: 'DELETE' });
    onBack();
  }
  ...
  if (err || !audit) {
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900">{err || 'No encontrado'}</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          >
            <CheckCircle2 className="h-3.5 w-3.5" /> {audit.paidAt ? 'Pagado ✓' : 'Marcar pagado'}
          </button>
          <button
          ...
          >
            <CheckCircle2 className="h-3.5 w-3.5" /> {audit.deliveredAt ? 'Entregado ✓' : 'Marcar entregado'}
          </button>
        </div>
      </div>
      ...
      {isRunning ? (
        <div className="rounded-2xl border border-sky-200 bg-sky-50 p-10 text-center">
          ...
          <p className="mt-3 text-sm font-semibold text-sky-800">Analizando el contenido…</p>
        ...
        <AeoReport result={audit.resultJson} showInternal />
      ) : null}
    </div>
  );
}
