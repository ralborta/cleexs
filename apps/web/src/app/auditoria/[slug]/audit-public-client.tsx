...
type PublicAudit = {
  ...
  result: AgenticAuditResult | null;
  generatedAt: string;
};

export function AuditPublicClient({ slug }: { slug: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/api/public/agentic-audit/${encodeURIComponent(slug)}`, {
        cache: 'no-store',
      });
      if (res.status === 404) {
        setNotFound(true);
      ...
      if (!res.ok) return;
      setAudit((await res.json()) as PublicAudit);
    } catch {
      /* ignore */
    } finally {
      setLoading(false);
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-4 md:px-6">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700 ring-1 ring-violet-200">
            <ScanSearch className="h-3.5 w-3.5" /> Auditoría Agéntica
          </span>
        </div>
      </header>
          ...
          <>
            ...
            {audit.status === 'running' || audit.status === 'pending' ? (
              <div className="rounded-2xl border border-sky-200 bg-sky-50 p-10 text-center">
                ...
                <p className="mt-3 text-sm font-semibold text-sky-800">Estamos generando tu auditoría…</p>
              ...
              <AgenticAuditReport result={audit.result} />
            ) : null}

            {/* CTA cierre */}
            {audit.status === 'completed' && (
              <div className="mt-8 rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-50 via-white to-indigo-50 p-6 text-center shadow-sm">
                <h2 className="text-base font-bold text-slate-900">
                  ¿Querés que te ayudemos a mejorar este score?
                </h2>
                <p className="mx-auto mt-1 max-w-xl text-sm text-slate-600">
                  En Cleexs medimos y optimizamos cómo te ven las IAs. Escribinos y armamos un plan para
                  que tu sitio gane tráfico desde ChatGPT, Gemini, Perplexity y los nuevos agentes.
                </p>
                <a
