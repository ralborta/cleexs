...
export type EmailEnviosDashboardProps = {
  mode?: 'admin' | 'portal';
  onGoTemplates?: () => void;
};

export function EmailEnviosDashboard({ mode = 'admin', onGoTemplates }: EmailEnviosDashboardProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [portalBatch, setPortalBatch] = useState('');
  const selectedSlug =
    mode === 'portal' ? portalBatch : searchParams.get('batch')?.trim() || '';
    ...
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          {onGoTemplates ? (
            <button type="button" onClick={onGoTemplates} className={secondaryBtn}>
              Ver plantillas
            </button>
          ) : (
