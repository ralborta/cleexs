...
export type EmailPlantillasDashboardProps = {
  mode?: 'admin' | 'portal';
  onGoEnvios?: () => void;
};

export function EmailPlantillasDashboard({ mode = 'admin', onGoEnvios }: EmailPlantillasDashboardProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [portalTab, setPortalTab] = useState<PageTab>('secuencia-free');

  const activeTab: PageTab = useMemo(() => {
    if (mode === 'portal') return portalTab;
