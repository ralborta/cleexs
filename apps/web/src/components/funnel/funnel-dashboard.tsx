...
export function buildAcquisitionStages(f: FunnelMetrics['funnel']): AcquisitionStage[] {
  return [
    {
      key: 'visitors',
      label: 'Visitantes',
              ...
              <div key={stage.key}>
                {index > 0 && prev ? (
                  <div className="mb-3 ml-10 flex items-center gap-2 py-1 text-[11px] sm:ml-12">
                    ...
                    <span className="font-medium text-slate-500">{pctLabel(advanced)} avanzó</span>
          ...
          <div className="flex h-full flex-col">
            ...
            {worstDrop.abandoned > 0 && worstDrop.dropPct > 0 ? (
              <p className="mt-2 text-sm leading-relaxed text-slate-700">
                ...
                {worstDrop.from.startsWith('Email') ? ' — después de dejar su email.' : '.'}
              </p>
            ) : (
...
function CumulativeConversionTimeline({
  ...
  d60,
  d90,
  purchaseLabel = 'Compras Plan Conquistar después del diagnóstico',
}: {
  eligible: number;
  h24: FunnelStep;
  d30: FunnelStep;
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="mb-6">
        ...
        <p className="mt-0.5 text-xs text-slate-500">
          {purchaseLabel} · {fmt(eligible)} emails elegibles
        </p>
      </div>
...
function ReferrersCampaignsTable({ rows }: { rows: FunnelMetrics['byReferrer'] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (!q) return true;
      return r.name.toLowerCase().includes(q) || r.refCode.toLowerCase().includes(q);
    });
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      ...
                      {!isOrganic ? <p className="text-[11px] text-slate-400">{row.refCode}</p> : null}
                    </td>
...
export type FunnelDashboardProps = {
  ...
  onPreset: (preset: 'hoy' | 'ayer' | '7' | '15' | '30') => void;
      ...
      {f ? (
        <>
          ...
          <p className="flex items-start gap-2 text-[11px] leading-relaxed text-slate-500">
            ...
            <span>
              {data?.notes.purchaseLinkageDetail} Elegibles cohorte: {fmt(data?.cohorts.eligible ?? 0)}{' '}
              emails únicos con diagnóstico en el rango.
            </span>
          </p>
        </>
      ) : null}
