...
export type ReferidoresFetcher = (path: string, init?: RequestInit) => Promise<Response>;

export type ReferidoresDashboardProps = {
  /** Base path sin trailing slash, ej. /api/admin-ui/referrals */
  apiBase: string;
  fetcher: ReferidoresFetcher;
  title?: string;
  subtitle?: string;
...
export function ReferidoresDashboard({
  apiBase,
  fetcher,
  title = 'Referidores',
  ...
  const load = useCallback(async () => {
    ...
    onError?.(null);
    try {
      const res = await fetcher(apiBase);
      ...
      const msg = e instanceof Error ? e.message : 'No se pudo cargar referidores';
      setError(msg);
      onError?.(msg);
      setDashboard(null);
    } finally {
  ...
  async function saveCampaign(e: React.FormEvent) {
    ...
    try {
      ...
      const path = form.id ? `${apiBase}/${encodeURIComponent(form.id)}` : apiBase;
  ...
  async function toggleCampaign(row: ReferralRow) {
    ...
    setError(null);
    try {
      const res = await fetcher(`${apiBase}/${encodeURIComponent(row.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
          ...
          <header className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                El link oficial será <code className="font-mono text-[10px]">/r/&lt;ref&gt;</code>
              </p>
            </div>
          </header>
          <form className="space-y-4 p-5" onSubmit={saveCampaign}>
            <label className="block">
              ...
              <span className="mt-1 block text-[11px] text-slate-500">
                Dónde compraste publicidad / quién comparte el link.
              </span>
            </label>
            ...
            <label className="block">
              ...
              />
              <span className="mt-1 block text-[11px] text-slate-500">
                Letras minúsculas, números, guión y guión bajo.
              </span>
            </label>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-3 text-right tabular-nums text-slate-700">
                          {formatNumber(row.diagnosticsStarted)}
                        </td>
                        <td className="px-3 py-3 text-right tabular-nums text-slate-700">
                          {formatNumber(row.completedDiagnostics)}
                        </td>
                        <td className="px-3 py-3 text-right">
                          <span
          ...
          <footer className="border-t border-slate-100 bg-slate-50/40 px-5 py-3 text-[11px] leading-relaxed text-slate-500">
            Emails únicos = personas distintas. Si un canal no tiene fila propia, comparte link con otro o el usuario
            entró sin <code className="rounded bg-slate-200 px-1">ref</code> en la URL (ver fila «Sin referidor» en
            Todos).
          </footer>
        </section>
      </div>
