...
export type EmailOutreachReportDashboardProps = {
  loadEmailOutreach?: (windowDays: ReportWindowDays) => Promise<EmailOutreachReport>;
  mode?: 'admin' | 'portal';
};

export function EmailOutreachReportDashboard({
  loadEmailOutreach,
  mode = 'admin',
}: EmailOutreachReportDashboardProps = {}) {
  ...
  const fetchEmailOutreach = loadEmailOutreach ?? internalReportsApi.emailOutreach;

  const load = useCallback(async () => {
    setError(null);
    try {
      const res = await fetchEmailOutreach(windowDays);
      setData(res);
    } catch (e) {
        ...
        <>
          ...
          {mode === 'admin' ? (
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600">
              <p className="flex items-center gap-2">
                <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />
                Demo Empliados · métricas ilustrativas de email post-demo y outreach a operadores.
              </p>
            </div>
          )}
        </>
